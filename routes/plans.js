...
function buildWhereClause(filters = {}) {
  ...
  if (filters.is_popular !== undefined) { conditions.push('p.is_popular = ?'); params.push(filters.is_popular ? 1 : 0); }
...
function buildOrderClause(sort) {
  if (!sort || typeof sort !== 'string') return 'ORDER BY p.sort_order ASC, p.price ASC';
  const isDesc = sort.startsWith('-');
  const field = isDesc ? sort.slice(1) : sort;
  if (!ALLOWED_SORT_FIELDS.includes(field)) return 'ORDER BY p.sort_order ASC, p.price ASC';
...
router.get('/api/plans', validate(paginationSchema), (req, res) => {
  try {
    const db = getDb();
    ...
    const dataSql = `SELECT p.* FROM plans p ${whereClause} ${orderClause} LIMIT ? OFFSET ?`;
...
router.get('/api/plans/:id', validate(paramsIdSchema), (req, res) => {
  try {
    const db = getDb();
    const { id } = req.params;
    const row = db.prepare('SELECT * FROM plans WHERE id = ?').get(id);
    if (!row) return res.status(404).json({ error: 'Plan not found.', code: 'NOT_FOUND' });
...
router.post(
  ...
  (req, res) => {
    try {
      const { name, slug, description, price, duration_days, features, is_popular, is_active, sort_order } = req.body;
      const db = getDb();
      const existing = db.prepare('SELECT id FROM plans WHERE slug = ?').get(slug);
      if (existing) return res.status(409).json({ error: 'A plan with this slug already exists.', code: 'SLUG_CONFLICT' });
      ...
      if (err.message && err.message.includes('UNIQUE constraint'))
        return res.status(409).json({ error: 'A plan with this slug already exists.', code: 'SLUG_CONFLICT' });
      return res.status(500).json({ error: 'Internal server error.', code: 'INTERNAL_ERROR' });
    }
  }
);
...
router.put(
  ...
  (req, res) => {
    try {
      ...
      if (req.body.duration_days !== undefined) { updates.duration_days = req.body.duration_days; setClauses.push('duration_days = ?'); }
      if (req.body.sort_order !== undefined) { updates.sort_order = req.body.sort_order; setClauses.push('sort_order = ?'); }
      if (req.body.features !== undefined) { updates.features = safeJsonStringify(req.body.features) || '[]'; setClauses.push('features = ?'); }
      if (req.body.is_popular !== undefined) { updates.is_popular = req.body.is_popular ? 1 : 0; setClauses.push('is_popular = ?'); }
      ...
      if (setClauses.length === 0) {
        ...
        return res.json({ message: 'No changes provided.', data: plan });
      }
      if (updates.slug !== undefined && updates.slug !== existing.slug) {
        const slugConflict = db.prepare('SELECT id FROM plans WHERE slug = ? AND id != ?').get(updates.slug, id);
        if (slugConflict) return res.status(409).json({ error: 'A plan with this slug already exists.', code: 'SLUG_CONFLICT' });
      }
      const now = new Date().toISOString().replace('T', ' ').slice(0, 19);
      ...
      return res.json({ message: 'Plan updated successfully.', data: plan });
    } catch (err) {
...
router.delete(
  ...
  (req, res) => {
    try {
      ...
      return res.json({ message: 'Plan deleted successfully.', deleted: { id: existing.id, name: existing.name } });
    } catch (err) {
