...
// ---------------------------------------------------------------------------
// Database health check (pre-flight)
// ---------------------------------------------------------------------------

/**
 * Verifică inițializarea bazei de date direct (fără HTTP), capturând
 * erori precum „Too many parameter values” care apar când SQLite primește
...
function checkDatabaseHealth() {
  try {
    ...
    // Detectează eroarea specifică „Too many parameter values”
...
async function runSuites() {
  ...
  if (dbHealth.ok) {
    ...
    console.log(`${C.red}FAIL${C.reset} Database initialization error:`);
    console.log(`${C.red}   → ${dbHealth.error}${C.reset}`);
    console.log(`\n${C.yellow}⚠️  Serverul nu a putut porni din cauza erorii de bază de date.${C.reset}`);
    console.log(`${C.yellow}   Remediați eroarea în config/db.js înainte de a rula testele HTTP.${C.reset}\n`);
    process.exit(1);
  }
